...
const DoctorProfilePage = ({ darkMode = false }: DoctorProfilePageProps) => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        const [me, doctorProfile] = await Promise.all([
          usersApi.me(),
          usersApi.getMyDoctorProfile(),
        ]);
        const nextDoctorId = formatDoctorId(
          me.id || storedProfile?.doctorId || "",
        );
        ...
        if (storedProfile) {
          ...
          setMessage(
            t("doctor", "profileLoaded", "Profile loaded from saved data."),
          );
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      writeStoredProfile(nextProfile);
      setMessage(
        t("doctor", "profileUpdated", "Profile updated successfully."),
      );
      setEditMode(false);
    } catch {
      setMessage(
        t(
          "doctor",
          "profileSaveFailed",
          "Unable to save doctor profile right now. Please try again later.",
        ),
      );
    } finally {
      setSaving(false);
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-[#0B3C5D] dark:bg-black text-white p-4 shadow-md gap-4">
        <div>
          <h2 className="text-3xl md:text-5xl font-bold ml-0 md:ml-5 md:pl-5 text-center md:text-left">
            {t("doctor", "myProfileTitle", "My Profile")}
          </h2>
        </div>
        <img
          src={doctorImg}
          alt={t("doctor", "doctorProfileAlt", "Doctor Profile")}
          ...
          <section className="rounded-3xl border-4 border-[#0B3C5D] bg-white dark:bg-black p-5 md:p-8 shadow-lg">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-sm text-[#4B5563] dark:text-gray-300">
                {loading
                  ? t("doctor", "loadingProfile", "Loading your profile...")
                  : ""}
              </div>
              <button
                type="button"
              ...
              >
                {!editMode && (
                  <img
                    src={editIcon}
                    alt={t("doctor", "edit", "Edit")}
                    className="w-4 h-4 object-cover rounded"
                    loading="lazy"
                  />
                )}
                {editMode
                  ? t("doctor", "saveChanges", "Save Changes")
                  : t("doctor", "edit", "Edit")}
              </button>
            </div>
