        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white dark:bg-black border-4 border-[#0B3C5D] rounded-2xl p-4">
              <h3 className="text-lg md:text-xl font-semibold text-[#0B3C5D] dark:text-white mb-4">
                Appointment Trends (Last 6 Months)
              </h3>
              <div className="flex items-end gap-2 h-40">
                {appointmentTrends.map((trend) => {
                  const maxCount = Math.max(
                    ...appointmentTrends.map((t) => t.count),
                  );
                  const barHeight =
                    maxCount > 0
                      ? `${Math.max(14, (trend.count / maxCount) * 100)}%`
                      : "14px";
                  return (
                    <div
                      key={trend.label}
              ...
              <div className="space-y-2">
                {recentAppointments.length > 0 ? (
                  recentAppointments.map((appointment) => (
                    <div
                      key={appointment.id}
                      className="border border-[#0B3C5D] rounded-2xl p-3"
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-semibold text-[#0B3C5D] dark:text-white">
                            {appointment.reason}
                          </p>
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            Patient:{" "}
                            {toPatientDisplayId(appointment.patient_id)} |{" "}
                            {new Date(
