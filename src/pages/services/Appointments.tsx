...
import editImg from "/medirator_images/edit.png";
...
const Appointments = ({ darkMode = false }: AppointmentsProps) => {
  ...
  const populateAppointmentForm = (appointment: {
    id: string;
    reason: string;
    date: string;
    time: string;
    ...
    const formattedDate = new Date(appointment.scheduledFor);
  ...
  const resetAppointmentForm = () => {
    ...
    setAppointmentDate("");
    setAppointmentTime("");
    setDoctorId((current) => current || doctorOptions[0]?.id || "");
    setReason("");
  };
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      if (editingAppointmentId) {
        await appointmentsApi.update(editingAppointmentId, {
                ...
                >
                  {loading
                    ? editingAppointmentId
                      ...
                      : t("auth", "booking", "Booking...")
                    : editingAppointmentId
                      ? t("auth", "updateAppointment", "Update Appointment")
