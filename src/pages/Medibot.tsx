  ...
  const loadSessionMessages = async (sessionId: string) => {
    ...
    try {
      ...
      const formatted: ChatMessage[] = msgs.map((m) => ({
        ...
        actualModelUsed: m.actual_model_used,
      ...
      if (currentChatIdRef.current === activeTargetChatId) {
        setMessages((prev) => [
          ...prev,
          {
            ...
            role: "bot",
            text: assistantReply,
            modelUsed: payload?.data?.model_used,
            actualModelUsed: payload?.data?.actual_model_used,
            confidence: payload?.data?.confidence,
            selectionReason: payload?.data?.selection_reason,
          },
        ]);
      }
          ...
          <div className="max-w-2xl mx-auto space-y-3">
            ...
            {messages.map((m) => (
              ...
              >
                ...
                >
                  <div>{m.text}</div>
                  {/* Model metadata (modelUsed/confidence/selectionReason) intentionally hidden */}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <textarea
                  ...
                  placeholder={
                    modelPreference === "xrayas"
                      ? "Upload an x-ray image instead of typing for XRayAS."
                      : "Ask Medibot..."
                  }
                  rows={1}
                  ...
                  } ${modelPreference === "xrayas" ? "opacity-50 cursor-not-allowed" : ""}`}
                />
              </div>
              ...
              <div ref={modelDropdownRef} className="relative mr-2">
                ...
                {modelDropdownOpen && (
                  ...
                  >
                    {[
                      { value: "gemini", label: "Medibot" },
                      { value: "xrayas", label: "xrayIT" },
                    ].map((option) => (
                      <button
                        key={option.value}
                        type="button"
                        ...
                        className={`border-[#0B3C5D] m-1 rounded-md transition-all duration-200 cursor-pointer flex items-center gap-2 px-3 py-2 w-full text-left ${
              ...
              <button
                ...
                title={
                  modelPreference === "xrayas"
                    ? "Upload a chest x-ray image for XRayAS analysis"
                    : "Upload file"
                }
              >
