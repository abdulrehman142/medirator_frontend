  ...
  useEffect(() => {
    const loadDoctors = async () => {
      try {
        ...
        setDoctors(
          Array.isArray(response) ? response.map(normalizeDoctor) : [],
        );
        setApiError(null);
      } catch {
  ...
  const filteredDoctors = useMemo(
    () =>
      doctors.filter((doctor) =>
        toLowerText(doctor.name).includes(searchQuery.toLowerCase()),
      ),
    [doctors, searchQuery],
  );
    ...
    try {
      ...
      setApiError(
        t(
          "admin",
          "doctorUpdateFailed",
          "Doctor update not confirmed by server.",
        ),
      );
    }
  };
  ...
  return (
    <div className={darkMode ? "dark" : ""}>
      <div className="flex flex-col md:flex-row justify-between items-center bg-[#0B3C5D] dark:bg-black text-white p-4 shadow-md gap-4">
        <div>
          <h2 className="text-3xl md:text-5xl font-bold ml-0 md:ml-5 md:pl-5 text-center md:text-left">
            {t("admin", "doctorManagementTitle", "Doctor Management")}
          </h2>
        </div>
        <img
          src={doctorImg}
          alt={t("admin", "doctorManagementTitle", "Doctor Management")}
      ...
      <div className="dark:bg-black px-3 md:px-6 py-6 space-y-4 font-sans text-black dark:text-white">
        ...
        {/* Add-new-doctor removed per admin UX update. Search is below. */}
        ...
        <section className="rounded-2xl border-4 border-[#0B3C5D] bg-white dark:bg-black p-4 md:p-6">
          <div className="flex justify-between gap-2 flex-col md:flex-row md:items-center">
            <h3 className="text-lg md:text-xl font-semibold text-[#0B3C5D] dark:text-white">
              {t("admin", "doctorAccounts", "Doctor Accounts")}
            </h3>
            <input
              value={searchQuery}
            ...
            <table className="w-full min-w-[980px] text-sm">
              <thead className="bg-[#0B3C5D] text-white">
                <tr>
                  ...
                  <th className="text-left px-4 py-3">
                    {t("admin", "specialization", "Specialization")}
                  </th>
                  <th className="text-left px-4 py-3">
