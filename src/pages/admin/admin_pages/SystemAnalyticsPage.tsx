...
const SystemAnalyticsPage = ({ darkMode = false }: SystemAnalyticsPageProps) => {
  ...
  const analyticsData = useMemo(() => {
    ...
    const completedValues = completedTrends.map((item) => Number(item.value) || 0);

    return {
      labels: analytics.patient_growth.map((item) => item.label),
      patientGrowth: analytics.patient_growth.map((item) => Number(item.value) || 0),
      appointments: analytics.appointment_trends.map((item) => Number(item.value) || 0),
  ...
  const renderBars = (values: number[], colorClassName: string, labels?: string[]) => {
    ...
    return numericValues.map((value, index) => {
      const heightPercent = isFinite(value) && isFinite(maxValue) ? (value / maxValue) * 100 : 0;
      return (
        <div key={`${value}-${index}`} className="flex-1 flex flex-col items-center justify-end h-full gap-2">
          <div className="w-full max-w-10 h-full flex items-end rounded-t-md border border-[#0B3C5D]/30 overflow-hidden">
            <div className={`w-full ${colorClassName}`} style={{ height: `${Math.max(heightPercent, 0)}%` }} />
          </div>
          <div className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400">{displayLabels[index] ?? ""}</div>
        </div>
      );
    });
  };
    ...
    <div className={darkMode ? "dark" : ""}>
      ...
      <div className="dark:bg-black px-3 md:px-6 py-6 space-y-4 font-sans text-black dark:text-white">
          ...
          <section className="rounded-2xl border-4 border-[#0B3C5D] bg-white dark:bg-black p-4 md:p-6">
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div className="rounded-2xl border border-[#0B3C5D] p-4 bg-[#f8fafc] dark:bg-[#0b1221]">
                <div className="text-xs text-gray-500 dark:text-gray-400">{t("admin", "totalUsers", "Total users")}</div>
                <div className="mt-2 text-3xl font-bold text-[#0B3C5D] dark:text-white">{formatNumber(dashboardMetrics.total_users)}</div>
              </div>
              <div className="rounded-2xl border border-[#0B3C5D] p-4 bg-[#f8fafc] dark:bg-[#0b1221]">
                <div className="text-xs text-gray-500 dark:text-gray-400">{t("admin", "totalAppointments", "Total appointments")}</div>
                <div className="mt-2 text-3xl font-bold text-[#0B3C5D] dark:text-white">{formatNumber(dashboardMetrics.total_appointments)}</div>
              </div>
              <div className="rounded-2xl border border-[#0B3C5D] p-4 bg-[#f8fafc] dark:bg-[#0b1221]">
                <div className="text-xs text-gray-500 dark:text-gray-400">{t("admin", "activeDoctors", "Active doctors")}</div>
                <div className="mt-2 text-3xl font-bold text-[#0B3C5D] dark:text-white">{formatNumber(dashboardMetrics.active_doctors)}</div>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div className="rounded-2xl border border-[#0B3C5D] p-4 bg-[#f8fafc] dark:bg-[#0b1221]">
                <div className="text-xs text-gray-500 dark:text-gray-400">{t("admin", "totalDoctors", "Total doctors")}</div>
                <div className="mt-2 text-3xl font-bold text-[#0B3C5D] dark:text-white">{formatNumber(analytics.totals.total_doctors)}</div>
              </div>
              <div className="rounded-2xl border border-[#0B3C5D] p-4 bg-[#f8fafc] dark:bg-[#0b1221]">
                <div className="text-xs text-gray-500 dark:text-gray-400">{t("admin", "totalPatients", "Total patients")}</div>
                <div className="mt-2 text-3xl font-bold text-[#0B3C5D] dark:text-white">{formatNumber(analytics.totals.total_patients)}</div>
              </div>
              <div className="rounded-2xl border border-[#0B3C5D] p-4 bg-[#f8fafc] dark:bg-[#0b1221]">
                <div className="text-xs text-gray-500 dark:text-gray-400">{t("admin", "completedAppointments", "Completed appointments")}</div>
                <div className="mt-2 text-3xl font-bold text-[#0B3C5D] dark:text-white">{formatNumber(analytics.totals.total_completed_appointments || 0)}</div>
              </div>
            </div>
          </section>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <section className="rounded-2xl border-4 border-[#0B3C5D] bg-white dark:bg-black p-4 md:p-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="text-sm text-gray-500 dark:text-gray-400">{t("admin", "recent7Days", "Last 7 days")}</span>
            </div>
            <div className="mt-4 h-44 flex items-end gap-2">{analyticsData ? renderBars(analyticsData.patientGrowth, "bg-[#0B3C5D]") : <div className="text-sm text-gray-500 dark:text-gray-400">{t("admin", "noAvailableData", "No available data.")}</div>}</div>
          </section>

          <section className="rounded-2xl border-4 border-[#0B3C5D] bg-white dark:bg-black p-4 md:p-6">
            <div className="flex items-center justify-between gap-4">
              <h3 className="text-lg font-semibold text-[#0B3C5D] dark:text-white">{t("admin", "appointmentTrends", "Appointments Registered")}</h3>
              <span className="text-sm text-gray-500 dark:text-gray-400">{t("admin", "recent7Days", "Last 7 days")}</span>
            </div>
            <div className="mt-4 h-44 flex items-end gap-2">{analyticsData ? renderBars(analyticsData.appointments, "bg-green-600") : <div className="text-sm text-gray-500 dark:text-gray-400">{t("admin", "noAvailableData", "No available data.")}</div>}</div>
          </section>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          <section className="rounded-2xl border-4 border-[#0B3C5D] bg-white dark:bg-black p-4 md:p-6">
            <h3 className="text-lg font-semibold text-[#0B3C5D] dark:text-white">{t("admin", "recentActivity", "Recent Activity")}</h3>
            <ul className="mt-4 space-y-2 text-sm">
              {recentActivity.map((activity, index) => (
                <li key={index} className="rounded-2xl border border-[#0B3C5D] p-3">
                  {activity}
                </li>
              ))}
              {recentActivity.length === 0 && (
                <li className="rounded-2xl border border-[#0B3C5D] p-3">{t("admin", "noAvailableData", "No available data.")}</li>
              )}
            </ul>
          </section>
        </div>
